...
shr = read.table(file="RNA-Seq.Liver/totalRNA.UCD/reconstruction/SHR.Liver.UCD.ensemblGuided.wMask/transcripts.gtf",sep="\t",header=FALSE)
shr = gtfFormat(shr)

bnlx=read.table(file="RNA-Seq.Liver/totalRNA.UCD/reconstruction/BNLx.Liver.UCD.ensemblGuided.wMask/transcripts.gtf",sep="\t",header=FALSE)
bnlx=gtfFormat(bnlx)
...
shr.tracking = read.table(file = "RNA-Seq.Liver/totalRNA.UCD/reconstruction/SHR.Liver.UCD.ensemblGuided.wMask/isoforms.fpkm_tracking",sep="\t",header=TRUE)
bnlx.tracking = read.table(file = "RNA-Seq.Liver/totalRNA.UCD/reconstruction/BNLx.Liver.UCD.ensemblGuided.wMask/isoforms.fpkm_tracking",sep="\t",header=TRUE)
...
# top gene is Rattus norvegicus RNA component of mitochondrial RNA processing endoribonuclease (Rmrp), RNase MRP RNA
# second gene is Rattus norvegicus RNA component of mitochondrial RNA processing endoribonuclease (Rmrp), RNase MRP RNA
# third gene is Rattus norvegicus retinol binding protein 4, plasma (Rbp4), mRNA


high.bnlx = high.bnlx[order(high.bnlx$FPKM,decreasing=TRUE),]
# top gene is Rattus norvegicus RNA component of mitochondrial RNA processing endoribonuclease (Rmrp), RNase MRP RNA
# second gene is Rattus norvegicus transthyretin (Ttr), mRNA
# third gene is Rattus norvegicus urinary protein 2 (Rup2), mRNA
```
...
if(printNow) write.table(shr.bed,file="RNA-Seq.Liver/totalRNA.UCD/reconstruction/tmp/shr.bed",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
if(printNow) write.table(bnlx.bed,file="RNA-Seq.Liver/totalRNA.UCD/reconstruction/tmp/bnlx.bed",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
...
#####################
overlap <- read.table(file="RNA-Seq.Liver/totalRNA.UCD/reconstruction/tmp/overlap.liver.total.byStrain.txt",sep="\t",header=FALSE)
...
##  examine one-exon transcripts - 1,242 overlaps with one-exon transcripts
...
## close match one-exon transcripts - 469 transcripts
...
##  create new transcript for matched transcripts - 2,774 matches
...
if(printNow) write.table(new.transcripts[,c("V1","start","stop","transcript_id","V6","V7")],file="RNA-Seq.Liver/totalRNA.UCD/reconstruction/tmp/combined.liver.total.06Oct14.bed",sep="\t",quote=FALSE,row.names=FALSE,col.names=FALSE)
```
...
export PATH=/usr/local/bedtools2/bin:$PATH
cd /data2/saba/BNLx.SHR/RNA-Seq.Liver/totalRNA.UCD/reconstruction/tmp
intersectBed -a combined.liver.total.06Oct14.bed -b combined.liver.total.06Oct14.bed -wo > overlap.liver.total.forGeneID.txt
```
...
overlap = read.table(file="RNA-Seq.Liver/totalRNA.UCD/reconstruction/tmp/overlap.liver.total.forGeneID.txt",sep="\t",header=FALSE)
...
if(printNow) write.table(gtf,file="RNA-Seq.Liver/totalRNA.UCD/reconstruction/reconstruct.total.liver.06Oct14.v1.gtf",row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
```
...
shr = read.table(file="quantitation/recon.Liver.total.v1.SHR/isoforms.fpkm_tracking",sep="\t",header=TRUE)
bnlx = read.table(file="quantitation/recon.Liver.total.v1.BNLx/isoforms.fpkm_tracking",sep="\t",header=TRUE)

## started wtih 21,033 transcripts  ##
...
## 18,862 (90%) transcripts with greater than 50 read coverage

gtf.old = read.table(file="quantitation/recon.Liver.total.v1.SHR/transcripts.gtf",header=FALSE,sep="\t")
...
if(printNow) write.table(new.gtf,file="reconstruction/reconstruct.liver.07Oct14.v2.gtf",row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
````
...
#rm(list=ls())
options(stringsAsFactors=FALSE)
setwd("/Volumes/saba/BNLx.SHR/RNA-Seq.Liver/")
printNow=FALSE

shr = read.table(file="totalRNA.UCD/quantitation/recon.Liver.total.v2.SHR/isoforms.fpkm_tracking",sep="\t",header=TRUE)
bnlx = read.table(file="totalRNA.UCD/quantitation/recon.Liver.total.v2.BNLx/isoforms.fpkm_tracking",sep="\t",header=TRUE)

## started wtih 18,862 transcripts  ##
...
## 18,833 (99.8%) transcripts with greater than 50 read coverage

gtf.old = read.table(file="totalRNA.UCD/quantitation/recon.Liver.total.v2.SHR/transcripts.gtf",header=FALSE,sep="\t")
gtf.old = gtfFormat(gtf.old)
...
if(printNow) write.table(new.gtf,file="totalRNA.UCD/reconstruction/reconstruct.liver.07Oct14.FINAL.gtf",row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
````
...
8. Quantitate FINAL Transcriptome
---------------------------------
```
#on PhenoGen server
cufflinks -u --seed 564654 -p 25 --library-type fr-firststrand -b /data2/saba/index/BNLx_rn5_wSpikesAndM.fa -G /data2/saba/BNLx.SHR/RNA-Seq.Liver/totalRNA.UCD/reconstruction/reconstruct.liver.07Oct14.FINAL.gtf -o /data2/saba/BNLx.SHR/RNA-Seq.Liver/totalRNA.UCD/quantitation/recon.Liver.total.FINAL.BNLx /hi-seq/BNLx.SHR.Liver.totalRNA/alignedReads/BNLx.liver.bam
cufflinks -u --seed 135646 -p 25 --library-type fr-firststrand -b /data2/saba/index/SHR_rn5_wSpikesAndM.fa -G /data2/saba/BNLx.SHR/RNA-Seq.Liver/totalRNA.UCD/reconstruction/reconstruct.liver.07Oct14.FINAL.gtf -o /data2/saba/BNLx.SHR/RNA-Seq.Liver/totalRNA.UCD/quantitation/recon.Liver.total.FINAL.SHR /hi-seq/BNLx.SHR.Liver.totalRNA/alignedReads/SHR.liver.bam
```
