...
Limit to transcripts with greater than 50 coverage and a transcript length greater than 200 nt (not including introns)
--------------
...
Identify overlap between novel transcripts in strain-specific reconstructions
---------------------------------
```{r,echo=FALSE}
...
closeMatch.oneExon[duplicated(closeMatch.oneExon$transcript_id.BNLx),]
closeMatch.oneExon[closeMatch.oneExon$transcript_id.BNLx=="BNLx.25607.1",]

#closeMatch.oneExon=closeMatch.oneExon[closeMatch.oneExon$transcript_id.BNLx!="ENSRNOT00000000660" & closeMatch.oneExon$transcript_id.SHR!="ENSRNOT00000000660",]
...
write.table(new.transcripts[,c("V1","start","stop","transcript_id","V6","V7")],file="RNA-Seq.Brain.polyA/reconstruction/tmp/combined.brain.polyA.21Jul14.bed",sep="\t",quote=FALSE,row.names=FALSE,col.names=FALSE)
```
...
sum(grepl("//",geneNames$geneNames,fixed=TRUE))
...
write.table(gtf,file="RNA-Seq.Brain.polyA/reconstruction/reconstruct.polyA.brain.21Jul14.v1.gtf",row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
```
...
length(unique(gtf.old$transcript_id))
length(unique(gtf.test$transcript_id))
...
write.table(new.gtf,file="reconstruction/reconstruct.polyA.brain.22Jul14.v2.gtf",row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
````
...
quant = quant[order(quant$gene_id,rowSums(quant[,grep("coverage",colnames(quant))])),]

##  Keep Only Transcripts with Greater Than 50 Read Coverage ##
keepers = quant[quant$coverage.shr>50 | quant$coverage.bnlx>50,]
nrow(keepers) / nrow(quant) 

######  Less than 1% of transcripts have less than 50 read coverage in at least one strain  ######

quant[quant$coverage.shr<=50 & quant$coverage.bnlx<=50,]

quant = quant[order(rowSums(quant[,grep("coverage",colnames(quant))]),decreasing=TRUE),]

numEns = aggregate(quant$tracking_id,by=list(gene_id = quant$gene_id),function(a) sum(grepl("ENSRN",a)))
colnames(numEns)[2] = "numEnsemblID"
quant = merge(quant,numEns,by="gene_id")


quant$category = NA
quant$category[grepl("ENSRN",quant$tracking_id)] = "ensembl transcript"
quant$category[quant$numEnsemblID>0 & !grepl("ENSRN",quant$tracking_id)] = "novel transcript for ensembl gene"
quant$category[quant$numEnsemblID==0 & grepl("match",quant$tracking_id)] = "novel transcript/gene found in both"
quant$category[quant$numEnsemblID==0 & grepl("BNLx",quant$tracking_id)] = "novel transcript/gene found originally in BNLx"
quant$category[quant$numEnsemblID==0 & grepl("SHR",quant$tracking_id)] = "novel transcript/gene found originally in SHR"
quant$category[quant$numEnsemblID==0 & grepl("ensMatch",quant$tracking_id)] = "close match to Ensembl Transcript"

table(quant$category,useNA="always")

head(quant[quant$category=="novel transcript/gene found in both",])
````
