...
## transcripts without any overlap - 6,362 transcripts
...
matched.undup$transcript_id = paste("matched",c(1:nrow(matched)),sep=".")
...
export PATH=/usr/local/bedtools2/bin:$PATH
cd /data/Tabastore3/LauraS/BNLx.SHR/RNA-Seq.Brain.polyA/reconstruction/tmp
intersectBed -a combined.brain.polyA.10Jul14.bed -b combined.brain.polyA.10Jul14.bed -wo > overlap.brain.polyA.forGeneID.txt
```
...
write.table(gtf,file="RNA-Seq.Brain.polyA/reconstruction/reconstruct.polyA.brain.10Jul14.v1.gtf",row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
```



```
qsub -q smp /home/data/saba/BNLx.SHR.Brain.polyARNA/programs/quant.Recon.10Jul14.v1.sh
qsub -q smp /home/data/saba/BNLx.SHR.Brain.polyARNA/programs/quant.Recon.10Jul14.v1.BNLx.sh
```
...
shr = read.table(file="quantification/recon.Brain.PolyA.v1/isoforms.fpkm_tracking",sep="\t",header=TRUE)
...
quant[quant$tracking_id=="ENSRNOT00000003715",]
quant[quant$gene_id=="GENE.2568",]
keepers = quant[quant$coverage.shr>40 | quant$coverage.bnlx>40,]

gtf.old = read.table(file="quantification/recon.Brain.PolyA.v1/transcripts.gtf",header=FALSE,sep="\t")
gtf.test = read.table(file="reconstruction/reconstruct.polyA.brain.10Jul14.v1.gtf",header=FALSE,sep="\t")
...
unique(gtf.test$transcript_id)[!(unique(gtf.test$transcript_id) %in% unique(gtf.old$transcript_id))]
