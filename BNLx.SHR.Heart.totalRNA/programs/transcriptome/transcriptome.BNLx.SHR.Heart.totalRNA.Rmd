...
bnlx = read.table(file = "RNA-Seq.Heart/totalRNA/reconstruction/BNLx.Heart.ensemblGuided.wMask/transcripts.gtf",sep="\t",header=FALSE)
bnlx = gtfFormat(bnlx)
...
bnlx.tracking = read.table(file = "RNA-Seq.Heart/totalRNA/reconstruction/BNLx.Heart.ensemblGuided.wMask/isoforms.fpkm_tracking",sep="\t",header=TRUE)
...
cd /data/Tabastore3/LauraS/BNLx.SHR/RNA-Seq.Heart/totalRNA/reconstruction/tmp
...
##  examine one-exon transcripts - 12,068 transcripts
...
if(printNow) write.table(new.transcripts[,c("V1","start","stop","transcript_id","V6","V7")],file="RNA-Seq.Heart/totalRNA/reconstruction/tmp/combined.heart.total.02Aug14.bed",sep="\t",quote=FALSE,row.names=FALSE,col.names=FALSE)
```
...
export PATH=/usr/local/bedtools2/bin:$PATH
cd /data/Tabastore3/LauraS/BNLx.SHR/RNA-Seq.Heart/totalRNA/reconstruction/tmp
intersectBed -a combined.heart.total.02Aug14.bed -b combined.heart.total.02Aug14.bed -wo > overlap.heart.total.forGeneID.txt
```
...
if(printNow) write.table(gtf,file="RNA-Seq.Heart/totalRNA/reconstruction/reconstruct.total.heart.02Aug14.v1.gtf",row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
```

6. Quantitate Combined Transcriptome (1st Iteration)
---------------------------------
```
qsub -q smp /storage/saba/BNLx.SHR.Heart.totalRNA/programs/quant.Recon.26Aug14.v1.SHR.sh
qsub -q smp /storage/saba/BNLx.SHR.Heart.totalRNA/programs/quant.Recon.26Aug14.v1.BNLx.sh
```

```{r,echo=FALSE}
#rm(list=ls())
options(stringsAsFactors=FALSE)
setwd("/Volumes/LauraS/BNLx.SHR/RNA-Seq.Brain.total/")
printNow=FALSE
