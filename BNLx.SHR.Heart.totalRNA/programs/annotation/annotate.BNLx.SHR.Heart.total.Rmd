...
setwd("/Volumes/saba/BNLx.SHR/RNA-Seq.Heart/totalRNA")
printNow=FALSE
...
gtf = gtfFormat(read.table(file="reconstruction/reconstruct.total.heart.07Oct14.FINAL.gtf",sep="\t",header=FALSE),FPKM=FALSE)
...
overlap = read.table(file="reconstruction/tmp/overlap.heart.total.wEnsembl.txt",sep="\t",header=FALSE)
...
# overlap between Ensembl transcripts and reconstruction
overlap = read.table(file="reconstruction/tmp/overlap.heart.total.wRefSeq.txt",sep="\t",header=FALSE)
overlap = overlap[overlap$V4!=overlap$V10,]
overlap = overlap[overlap$V6=="." | overlap$V12=="." | overlap$V6==overlap$V12,]
...
# overlap between Ensembl transcripts and reconstruction
overlap = read.table(file="reconstruction/tmp/overlap.heart.total.wMouseRefSeq.txt",sep="\t",header=FALSE)
overlap = overlap[overlap$V4!=overlap$V10,]
overlap = overlap[overlap$V6=="." | overlap$V12=="." | overlap$V6==overlap$V12,]
...
shr = read.table(file="quantitation/recon.Heart.total.FINAL.SHR/isoforms.fpkm_tracking",sep="\t",header=TRUE)
bnlx = read.table(file="quantitation/recon.Heart.total.FINAL.BNLx/isoforms.fpkm_tracking",sep="\t",header=TRUE)
...
if(printNow) save(withAnno,file="reconstruction/Rdata/withAnno.heart.total.Rdata")
...
top10[!is.na(top10$geneID) & top10$geneID=="NR_002905",c("Description","GeneSymbol")] = c("Mus musculus small nucleolar RNA, H/ACA box 74A (Snora74a), small nucleolar RNA","Snora74a")
...
if(printNow) write.table(unanno.gtf[,paste("V",1:9,sep="")],file="reconstruction/unannotated.heart.08Oct14.gtf",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
```
