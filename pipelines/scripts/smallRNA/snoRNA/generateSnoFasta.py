...
def parseOptions():
	usage = 'USAGE: %prog snoRNAListFile ssgFastaFile outputFile\n'
...
while(EOF==0):
	...
	if(len(line)>0):
		...
		if(len(col[3][3:])<3):
			#print (line+"\n")
			seq=fasta.getChrRange(col[3][3:],int(col[4]),int(col[5]),int(col[6]))

			if(len(seq)>0):
				fastaFile.write("> "+name+" | "+col[3]+":"+col[4]+":"+col[5]+":"+col[6]+"\n")
